...
SELECT year, region,
ROUND(AVG(CASE WHEN month IN ("1","2","3") THEN kwh_per_acc+0.0 END),4) AS Q1,
ROUND(AVG(CASE WHEN month IN ("4","5","6") THEN kwh_per_acc+0.0 END),4) AS Q2,
ROUND(AVG(CASE WHEN month IN ("7","8","9") THEN kwh_per_acc+0.0 END),4) AS Q3,
ROUND(AVG(CASE WHEN month IN ("10","11","12") THEN kwh_per_acc+0.0 END),4) AS Q4
FROM householdelectricityconsumption
GROUP BY year, region
...
SELECT
AVG(CASE WHEN month IN ("1","2","3") THEN kwh_per_acc+0.0 END) AS Q1,
AVG(CASE WHEN month IN ("4","5","6") THEN kwh_per_acc+0.0 END) AS Q2,
AVG(CASE WHEN month IN ("7","8","9") THEN kwh_per_acc+0.0 END) AS Q3,
AVG(CASE WHEN month IN ("10","11","12") THEN kwh_per_acc+0.0 END) AS Q4
FROM householdelectricityconsumption;
## Electrical consumption is on average highest during Q2 followed by Q3, followed by Q4 and lastly Q1
